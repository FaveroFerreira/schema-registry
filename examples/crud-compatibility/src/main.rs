use schema_registry::api::{CompatibilityAPI, ConfigurationAPI, SubjectAPI};
use schema_registry::client::SchemaRegistryClient;
use schema_registry::types::{CompatibilityLevel, SchemaType, SubjectConfig, UnregisteredSchema, Version};
...
async fn main() -> anyhow::Result<()> {
    ...
    client.post_new_subject_version(SUBJECT, &unregistered_schema, NORMALIZE).await?;


    let forward_compatible_schema = UnregisteredSchema::schema(FORWARD_COMPATIBLE_SCHEMA).schema_type(SchemaType::Avro);
    let is_forward_compatible = client.is_compatible(SUBJECT, Version::Latest, &forward_compatible_schema).await?;

    // by default, the schema is BACKWARD compatible, so forwards compatibility should be false
    assert!(!is_forward_compatible, "Forward compatibility should be false");

    let backward_compatible_schema = UnregisteredSchema::schema(BACKWARD_COMPATIBLE_SCHEMA).schema_type(SchemaType::Avro);
    let is_backward_compatible = client.is_compatible(SUBJECT, Version::Latest, &backward_compatible_schema).await?;

    // by default, the schema is backward compatible, so backwards compatibility should be true
    assert!(is_backward_compatible, "Backward compatibility should be true");
    ...
    client.update_subject_configuration(SUBJECT, &config).await?;

    let is_forward_compatible = client.is_compatible(SUBJECT, Version::Latest, &forward_compatible_schema).await?;

    // now the schema is FORWARD compatible, so forwards compatibility should be true
    assert!(is_forward_compatible, "Forward compatibility should be true");
